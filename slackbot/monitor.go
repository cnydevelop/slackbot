...
func Monitor(ctx context.Context, projectId string, buildId string, webhook string, project string, commitId string) {
	svc := gcbClient(ctx)
	errors := 0
	started := false
	...
	for {
		...
		case "WORKING":
			if !started {
				...
				Notify(monitoredBuild, webhook, project, commitId)
				started = true
			}
